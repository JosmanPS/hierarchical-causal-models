...
data.elecciones[ ,1] <- as.numeric(data.elecciones[ ,1])
data.elecciones[ ,2] <- as.numeric(data.elecciones[ ,2])
...
data.elecciones <- data.elecciones %>% filter(Ano == 2009)

# Unimos los datos con poblacion
metro.areas <- metro.areas[, c(2,4,5)]
names(metro.areas)[2:3] <- c('Estado', 'Municipio')
data <- inner_join(data.elecciones, metro.areas, by = c('Estado', 'Municipio'))

# Modificamos las muertes per capita
data <- data %>% mutate(MuertesPC = Muertes / mun_population_2010 * 1000)
